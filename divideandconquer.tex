...
\end{align}
The first case $O(n^{\lg_b a - \epsilon})$ is occurs when the \textit{divide}
...
the third case $O(n^{\lg_b a + \epsilon})$, which is symmetric. The second case
$\Theta(n^{\lg_b a})$ occurs when both steps, the \textit{divide} and
